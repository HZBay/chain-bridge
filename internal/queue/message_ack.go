...
// ackAllMessages acknowledges all messages after successful processing
func (c *RabbitMQBatchConsumer) ackAllMessages(messages []*MessageWrapper) {
	successCount := 0
	for _, msgWrapper := range messages {
		err := msgWrapper.Delivery.Ack(false)
		if err != nil {
			log.Error().Err(err).
				Str("message_id", msgWrapper.Delivery.MessageId).
	...
	log.Info().
		...
		Msg("Message ACK completed")
}

// nackAllMessages rejects all messages for retry or dead letter queue
func (c *RabbitMQBatchConsumer) nackAllMessages(messages []*MessageWrapper) {
	for _, msgWrapper := range messages {
		err := msgWrapper.Delivery.Nack(false, true) // requeue=true for retry
		if err != nil {
			log.Error().Err(err).
				Str("message_id", msgWrapper.Delivery.MessageId).
	...
	log.Warn().
		...
		Msg("Messages NACKed for retry")
}

// createBatchRecord creates a record in the batches table
func (c *RabbitMQBatchConsumer) createBatchRecord(
	tx *sql.Tx,
	batchID uuid.UUID,
	group BatchGroup,
	jobs []BatchJob,
	...
	_, err := tx.Exec(query,
		...
		"rabbitmq_batch",      // batch_strategy
		"medium",              // network_condition
		result.GasUsed,        // actual_gas_used
		result.GasSaved,       // gas_saved
		result.Efficiency,     // gas_saved_percentage
...
func (c *RabbitMQBatchConsumer) updateTransactionStatuses(
	tx *sql.Tx,
	jobs []BatchJob,
	batchID uuid.UUID,
	result *blockchain.BatchResult,
	...
	log.Debug().
		...
		Str("tx_hash", result.TxHash).
		Float64("efficiency", result.Efficiency).
		Msg("Updated transaction statuses to confirmed")

	return nil
}

// updateUserBalances updates user balance records
func (c *RabbitMQBatchConsumer) updateUserBalances(
	tx *sql.Tx,
	jobs []BatchJob,
	group BatchGroup,
) error {
	// Calculate balance changes for each user
	balanceChanges := c.calculateBalanceChanges(jobs, group.JobType)
	...
	for userID, change := range balanceChanges {
		err := c.updateSingleUserBalance(tx, userID, group.ChainID, group.TokenID, change)
		if err != nil {
...
func (c *RabbitMQBatchConsumer) calculateBalanceChanges(jobs []BatchJob, jobType JobType) map[string]*big.Int {
	...
	for _, job := range jobs {
		...
		case JobTypeAssetAdjust:
			if adjustJob, ok := job.(AssetAdjustJob); ok {
				...
				if adjustJob.AdjustmentType == "mint" {
					...
					c.addToBalance(changes, adjustJob.UserID, amount)
				} else if adjustJob.AdjustmentType == "burn" {
					// Burn decreases balance
					negAmount := new(big.Int).Neg(amount)
					c.addToBalance(changes, adjustJob.UserID, negAmount)
				}
			}
		...
		case JobTypeTransfer:
			if transferJob, ok := job.(TransferJob); ok {
				...
				negAmount := new(big.Int).Neg(amount)
				c.addToBalance(changes, transferJob.FromUserID, negAmount)

				// Receiver's balance increases
				c.addToBalance(changes, transferJob.ToUserID, amount)
			}
		}
	}
...
func (c *RabbitMQBatchConsumer) addToBalance(changes map[string]*big.Int, userID string, amount *big.Int) {
...
func (c *RabbitMQBatchConsumer) updateSingleUserBalance(
	tx *sql.Tx,
	userID string,
	chainID int64,
	tokenID int,
...
func (c *RabbitMQBatchConsumer) hasMintJobs(jobs []BatchJob) bool {
	for _, job := range jobs {
		if adjustJob, ok := job.(AssetAdjustJob); ok {
			if adjustJob.AdjustmentType == "mint" {
				return true
