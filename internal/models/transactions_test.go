...
var (
	transactionDBTypes = map[string]string{`ID`: `integer`, `TXID`: `uuid`, `OperationID`: `uuid`, `UserID`: `uuid`, `ChainID`: `bigint`, `TXType`: `enum.tx_type('mint','burn','transfer')`, `BusinessType`: `enum.business_type('transfer','reward','gas_fee','consumption','refund')`, `RelatedUserID`: `uuid`, `TransferDirection`: `enum.transfer_direction('outgoing','incoming')`, `TokenID`: `integer`, `Amount`: `numeric`, `AmountUsd`: `numeric`, `Status`: `enum.transaction_status('pending','batching','submitted','confirmed','failed')`, `TXHash`: `character`, `BlockNumber`: `bigint`, `BatchID`: `uuid`, `IsBatchOperation`: `boolean`, `GasSavedPercentage`: `numeric`, `ReasonType`: `character varying`, `ReasonDetail`: `text`, `Metadata`: `jsonb`, `CreatedAt`: `timestamp with time zone`, `ConfirmedAt`: `timestamp with time zone`, `SourceRecordID`: `character varying`}
	_                  = bytes.MinRead
)
