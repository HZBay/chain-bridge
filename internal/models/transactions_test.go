...
var (
	transactionDBTypes = map[string]string{`ID`: `integer`, `TXID`: `uuid`, `OperationID`: `uuid`, `UserID`: `text`, `ChainID`: `bigint`, `TXType`: `enum.tx_type('mint','burn','transfer','nft_mint','nft_burn','nft_transfer')`, `BusinessType`: `enum.business_type('transfer','reward','gas_fee','consumption','refund','burn','open_box')`, `RelatedUserID`: `text`, `TransferDirection`: `enum.transfer_direction('outgoing','incoming')`, `TokenID`: `integer`, `Amount`: `numeric`, `AmountUsd`: `numeric`, `Status`: `enum.transaction_status('pending','batching','submitted','confirmed','failed')`, `TXHash`: `character`, `BlockNumber`: `bigint`, `BatchID`: `uuid`, `IsBatchOperation`: `boolean`, `GasSavedPercentage`: `numeric`, `ReasonType`: `character varying`, `ReasonDetail`: `text`, `Metadata`: `jsonb`, `CreatedAt`: `timestamp with time zone`, `ConfirmedAt`: `timestamp with time zone`, `FailureReason`: `character varying`, `UpdatedAt`: `timestamp with time zone`, `CollectionID`: `character varying`, `NFTTokenID`: `character varying`, `NFTMetadata`: `jsonb`}
	_                  = bytes.MinRead
)
