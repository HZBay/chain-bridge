...
var (
	batchDBTypes = map[string]string{`ID`: `integer`, `BatchID`: `uuid`, `ChainID`: `bigint`, `TokenID`: `integer`, `BatchType`: `enum.batch_type('mint','burn','transfer','nft_mint','nft_burn','nft_transfer')`, `OperationCount`: `integer`, `OptimalBatchSize`: `integer`, `ActualEfficiency`: `numeric`, `BatchStrategy`: `character varying`, `NetworkCondition`: `enum.network_condition('low','medium','high')`, `ActualGasUsed`: `bigint`, `GasSaved`: `bigint`, `GasSavedPercentage`: `numeric`, `GasSavedUsd`: `numeric`, `CpopOperationType`: `enum.cpop_operation_type('batch_mint','batch_burn','batch_transfer','batch_nft_mint','batch_nft_burn','batch_nft_transfer')`, `MasterAggregatorUsed`: `boolean`, `Status`: `enum.batch_status('preparing','submitted','confirmed','failed')`, `TXHash`: `character`, `CreatedAt`: `timestamp with time zone`, `ConfirmedAt`: `timestamp with time zone`, `Confirmations`: `integer`, `ConfirmedBlock`: `bigint`, `RetryCount`: `integer`, `FailureReason`: `character varying`, `SubmittedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`}
	_            = bytes.MinRead
)
